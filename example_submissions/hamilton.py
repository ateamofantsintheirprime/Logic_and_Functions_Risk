...
def handle_place_initial_troop(game: Game, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    enforcement_targets = bot_state.war_focus & my_territories
    print("war focus:",bot_state.war_focus)
    print("my_territories:",my_territories)
    print("enforcement_targets:",enforcement_targets)
    if len(enforcement_targets) == 0:
        enforcement_targets = set(game.state.get_all_border_territories(bot_state.war_focus)) & my_territories
        print("adjacent to war focus:", set(game.state.get_all_border_territories(bot_state.war_focus)))
        if len(enforcement_targets) == 0:
            # I dont think this should be possible
            raise Exception
    
    # For now we will just try and create a big death stack to take our war focus. in future we should try and 
    # Balance taking the war focus, and also preemptively reinfocing the choke points if our competitors are not
    # ambitious within the war focus.
...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    # We should add troops to territories based on their threat level
    border_territories = set(game.state.get_all_border_territories(
        game.state.get_territories_owned_by(game.state.me.player_id)
    )) & bot_state.defense_focus

    reinforcement_priorities = sorted(list(border_territories), key= lambda x: threat(game,x), reverse=True)
    for territory in reinforcement_priorities:
