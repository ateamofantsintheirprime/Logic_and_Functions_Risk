...
def handle_place_initial_troop(game: Game, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    if game.state.territories[strongest_territory].troops == 1:
        strongest_territory = min(my_territories, key=lambda x: len(game.state.get_all_adjacent_territories([x])))
    return game.move_place_initial_troop(query, strongest_territory)
...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    distributions[strongest_territory] += total_troops
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    strongest_territory = max(my_territories, key=lambda x: game.state.territories[x].troops)
    bordering_territories = game.state.get_all_adjacent_territories(strongest_territory)

    def attack_if_possible(territories: list[int]):

        if set(bordering_territories).issubset(my_territories):
            return False
